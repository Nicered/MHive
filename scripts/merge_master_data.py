...
def main():
    ...
    # 개별 인시던트 파일 저장
    print("\n📁 개별 인시던트 파일 저장 중...")
    for inc in tqdm(merged, desc="파일 저장"):
        inc_id = inc['id']
        inc_file = os.path.join(incidents_dir, f"{inc_id}.json")
    ...
    for inc in merged:
        index_data.append({
            ...
            'summary': inc.get('summary', '')[:200],  # 요약만
            'tags': inc.get('tags', [])[:5],  # 상위 5개 태그
            'coordinates': inc.get('coordinates'),
    ...
    print(f"✅ 마스터 데이터 저장: {master_file}")

    # TypeScript 데이터 파일 생성 (JSON 임포트 방식 - 대용량 데이터 지원)
    ts_output = os.path.join(SCRIPTS_DIR, '..', 'lib', 'data.ts')

    ts_content = f'''// Auto-generated data loader
// Generated at: {datetime.now().isoformat()}
// Total incidents: {len(clean_incidents)}
// Total relations: {len(relations)}
// For large datasets, we use JSON import with type assertion

import {{ IncidentsData, Incident, Relation }} from "./types";

// Import the JSON data
import masterData from "../data/master/master_incidents.json";

// Extract and type-assert the data
const incidents = masterData.incidents as unknown as Incident[];
const relations = masterData.relations as unknown as Relation[];
